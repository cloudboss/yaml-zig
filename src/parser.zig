...
pub const Parser = struct {
    ...
    fn collectComments(self: *Parser) !?*const ast.CommentGroupNode {
        var comments = std.ArrayList(*const ast.CommentNode).empty;
        while (self.pos < self.tokens.len and
            self.tokens[self.pos].token_type == .comment)
        {
    ...
    fn mergeComments(
        ...
        if (a == null) return b;
        if (b == null) return a;
        var all = std.ArrayList(*const ast.CommentNode).empty;
    ...
    fn parseMappingWithFirst(
        ...
        var entries = std.ArrayList(*const ast.MappingValueNode).empty;
